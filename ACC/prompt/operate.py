...
# 可用工具
您必须使用以下工具或工具包进行所有操作，不要尝试直接操作系统：
{tools_description}
...
5. 想要查询如何调用一个工具，请使用search_tool操作。
6. 当你需要使用工具时，你必须使用search_tool操作查询工具的调用方法，然后使用tool操作调用工具。
...
"action_type": "tool/search_tool/history/none",  // 操作类型，"tool"表示使用工具，"search_tool"表示查询工具的调用方法，"history"表示读取已完成步骤的历史记录，"none"表示无操作
"use_tool": "想要查询调用方法的工具名称（如果"action_type": "search_tool"，该项为必填，其他情况禁止填写）",
...
3. 如果工具执行失败，请分析失败原因，并提供解决方案并再次尝试
...
SEARCH_TOOL_PROMPT = """以下是你请求的工具或工具包的详细调用方法：
{tool_info}
请根据你获取到的信息来使用工具
