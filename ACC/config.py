...
CONFIG_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "config", "config.toml"
)
...
def get_default_workspace_path():
    ...
    Returns:
        str: 默认工作空间路径，如果未配置则返回"workspace"
    """
    try:
        from ACC.config import config

        return config.get("workspace", {}).get("default_path", "workspace")
    except Exception:
        return "workspace"
