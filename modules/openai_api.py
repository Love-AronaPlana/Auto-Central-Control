...
class OpenAIAPI:
    ...
    def send_request(self, messages: List[Dict[str, Any]], tools: Optional[List[Dict[str, Any]]] = None) -> Optional[Dict[str, Any]]:
        """
        发送请求到OpenAI API
        ...
        if self.debug:
            ...
            logger.debug(f"发送请求到OpenAI API: {json.dumps(debug_payload, ensure_ascii=False)}")
        ...
        try:
            ...
            if self.debug:
                logger.debug(f"收到OpenAI API响应: {json.dumps(result, ensure_ascii=False)}")
