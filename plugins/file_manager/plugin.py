...
def read_file(file_path: str, encoding: str = "utf-8") -> Dict[str, Any]:
    ...
    if not allow_outside:
        # 检查路径是否在允许列表内
        allowed = any(os.path.abspath(p) in abs_path for p in allowed_paths)
        if not allowed:
            return {
                "status": "failed",
                "error": f"路径 {file_path} 不在允许的操作目录内",
            }
...
def append_file(
    ...
    try:
        ...
        return {"status": "error", "message": f"追加文件失败: {str(e)}"}
...
def delete_file(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "message": f"文件删除成功: {file_path}"}
    except Exception as e:
...
def list_files(directory: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not os.path.isdir(directory):
            return {"status": "error", "message": f"路径不是目录: {directory}"}

        files = []
        for item in os.listdir(directory):
            item_path = os.path.join(directory, item)
            item_type = "directory" if os.path.isdir(item_path) else "file"
            files.append({"name": item, "path": item_path, "type": item_type})

        return {"status": "success", "directory": directory, "files": files}
    except Exception as e:
        return {"status": "error", "message": f"列出文件失败: {str(e)}"}
