...
class WorkflowExecutor:
    ...
    def execute(self, user_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.memory.add_system_message(json.dumps(steps, ensure_ascii=False, indent=2))
            ...
            while current_step_index < total_steps:
                ...
                self.logger.info(f"开始执行步骤 {current_step_index + 1}/{total_steps}: {current_step['description']}")
                
                # 步骤2: 使用分析Agent确定如何执行当前步骤
                self.logger.info("步骤2: 分析如何执行当前步骤")
                execution_plan = self.analysis_agent.process(current_step, self.memory.get_conversation_history())
                self.memory.add_system_message("分析Agent已确定执行计划：")
                self.memory.add_system_message(json.dumps(execution_plan, ensure_ascii=False, indent=2))
                
                # 步骤3: 使用执行Agent执行当前步骤
                self.logger.info("步骤3: 执行当前步骤")
                execution_result = self.execution_agent.process(execution_plan, self.memory.get_conversation_history())
                self.memory.add_system_message("执行Agent已完成步骤执行：")
                self.memory.add_system_message(json.dumps(execution_result, ensure_ascii=False, indent=2))
                
                # 步骤4: 使用复查Agent检查执行结果
                self.logger.info("步骤4: 复查执行结果")
                review_result = self.review_agent.process(execution_result, current_step, self.memory.get_conversation_history())
                self.memory.add_system_message("复查Agent已检查执行结果：")
                self.memory.add_system_message(json.dumps(review_result, ensure_ascii=False, indent=2))
                ...
                if review_result.get('has_errors', False):
                    ...
                    while review_result.get('has_errors', False) and fix_attempts < max_fix_attempts:
                        ...
                        # 步骤5: 使用修复Agent修复错误
                        fix_result = self.fix_agent.process(review_result, self.memory.get_conversation_history())
                        self.memory.add_system_message(f"修复Agent已尝试修复错误（第{fix_attempts}次）：")
                        self.memory.add_system_message(json.dumps(fix_result, ensure_ascii=False, indent=2))
                        
                        # 再次复查
                        review_result = self.review_agent.process(fix_result, current_step, self.memory.get_conversation_history())
                        self.memory.add_system_message(f"复查Agent已再次检查执行结果（第{fix_attempts}次）：")
                        self.memory.add_system_message(json.dumps(review_result, ensure_ascii=False, indent=2))
                    
                    if review_result.get('has_errors', False):
                        self.logger.warning(f"在{max_fix_attempts}次尝试后仍无法修复所有错误，继续执行下一步骤")
            ...
            summary_result = self.summary_agent.process(steps, self.memory.get_conversation_history())
            self.memory.add_system_message("结束Agent已总结执行结果：")
            self.memory.add_system_message(json.dumps(summary_result, ensure_ascii=False, indent=2))
...
# 工作流执行函数，用于从外部调用
def execute_workflow(config: Dict[str, Any], user_input: str, logger: logging.Logger | None = None) -> Dict[str, Any]:
    """
    执行工作流程的便捷函数
