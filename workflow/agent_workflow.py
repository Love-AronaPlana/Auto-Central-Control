...
from config.constants import (
    STATUS_IDLE, STATUS_ANALYZING, STATUS_SELECTING_TOOL,
    STATUS_EXECUTING, STATUS_PROCESSING_RESULT, STATUS_COMPLETED
)
...
class AgentWorkflow:
    ...
    async def process_result(self, execution_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 添加处理结果到消息历史
            self.add_assistant_message(json.dumps(processing_result, ensure_ascii=False))
    ...
    async def run(self, user_input: str) -> str:
        ...
        # 检查是否需要继续执行
        if processing_result.get("next_actions") and len(processing_result["next_actions"]) > 0:
            # 递归执行下一步
