            ...
            if os.path.isdir(plugin_path) and not item.startswith("_"):
                try:
                    ...
                    if plugin_info:
                        ...
                        self.logger.info(
                            f"加载插件: {item} v{plugin_info.get('version', '1.0.0')}"
                        )
...
"""
    ...
    def process(
        self, execution_plan: Dict[str, Any], conversation_history: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        处理执行计划，执行操作
    ...
    def _execute_plugin(
        ...
        try:
            ...
            if plugin_name not in self.available_plugins:
                return {"status": "failed", "error": f"插件 {plugin_name} 不存在"}
            ...
            return {"status": "failed", "error": f"执行失败: {str(e)}"}
